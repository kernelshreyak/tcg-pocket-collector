<script>
  ...
  function cardImage() {
    ...
    if (isDefined(set) && isDefined(number)) {
      return `https://images.pokemontcg.io/${set.id.toLowerCase()}/${number}_hires.png`;
    }
    return "";
  }
  ...
  function foilMaskImage(prop, type = "masks") {
    ...
    // map foil and mask filenames (to allow max compatibility with other sets)
    const styleFoilMaskMap = {
      rainbow: "foils/swsh12/masks/upscaled/tg02_foil_holo_rainbow_2x.webp",
      cosmos: "foils/swshp/masks/upscaled/127_foil_holo_cosmos_2x.webp",
      reverse: "foils/swsh12/foils/upscaled/085_foil_holo_reverse_2x.webp",
      radiantholo:
        "foils/swsh12/masks/upscaled/059_foil_etched_radiantholo_2x.webp",
      swholo: "foils/swsh1/masks/upscaled/085_foil_holo_swholo_2x.webp",
      swsecret: "foils/swsh4/foils/upscaled/050_foil_etched_swsecret_2x.webp",
      etched_sunpillar:
        "foils/swsh8/masks/upscaled/250_foil_etched_sunpillar_2x.webp",
      holo_sunpillar:
        "foils/swsh12/masks/upscaled/049_foil_holo_reverse_2x.webp",
    };

    let mapkey = styleFoilMaskMap[style];
    if (style == "sunpillar") {
      mapkey = styleFoilMaskMap[`${etch}_${style}`];
    }
    if (type == "foil") {
      mapkey = mapkey.replace("masks", "foils");
    }
    return `${server}/${mapkey}`;
  }
